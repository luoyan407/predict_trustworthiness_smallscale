...
import sys
sys.path.append('/home/yluo/project/python/oracle_learning/ConfidNet')
...
def main():
    ...
    parser.add_argument("--boundary", "-b", type=float, default=0.0, help="decision boundary")
    parser.add_argument(
        "--mode",
        "-m",
        type=str,
    ...
    test_ckpt_path = config_args["model"].get("test_model", None)
    test_checkpoint = checkpoint
    if test_ckpt_path:
        test_checkpoint = torch.load(test_ckpt_path)

    # Get scores
    LOGGER.info(f"Inference mode: {args.mode}")

    npz_file = os.path.join(config_args["training"]["output_folder"], f'test_{args.mode}.npz')
    if args.mode != "trust_score":
        # learner.model.load_state_dict(test_checkpoint["model_state_dict"])
