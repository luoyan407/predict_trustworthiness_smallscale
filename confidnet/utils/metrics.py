...
class Metrics:
    ...
    def get_scores(self, split="train", tn_th=-1):
        # if self.accurate == [] or self.errors == [] or self.proba_pred == []:
        #     return {}
        ...
        if "auc" in self.metrics:
            ...
            # scores[f"{split}/auc"] = {"valudloadere": auc_score, "string": f"{auc_score:05.2%}"}
        ...
        if "tpr" in self.metrics:
            tpr = np.sum((self.accurate == 1) & (self.proba_pred >= 0.5)) / np.sum(self.accurate == 1)
            scores[f"{split}/tpr"] = {"value": tpr, "string": f"{tpr*100:02.2f}"}
        if "tnr" in self.metrics:
            if tn_th < 0:
                tn_th = 1.0/self.n_classes
            tnr = np.sum((self.errors == 1) & (self.proba_pred < tn_th)) / np.sum(self.errors == 1)
            scores[f"{split}/tnr"] = {"value": tnr, "string": f"{tnr*100:02.2f}"}
        return scores

    def save_to_npz(self, file):
        # self.accurate = np.reshape(self.accurate, newshape=(len(self.accurate), -1)).flatten()
        # self.errors = np.reshape(self.errors, newshape=(len(self.errors), -1)).flatten()
        # self.proba_pred = np.reshape(self.proba_pred, newshape=(len(self.proba_pred), -1)).flatten()
        np.savez(file, accurate=self.accurate, errors=self.errors, tw=self.proba_pred)
        return